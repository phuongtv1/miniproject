...
<script>
import { isValidEmail } from '@/mixins/validators';
...
export default {
  ...
  computed: {
    ...mapGetters(['isUserSignedUp', 'isSignupModalOpen']),
  ...
  methods: {
    ...
    ...mapActions(['showSignupModal', 'checkIsUserSignedUpStatus', 'checkIsUserLoggedInStatus']),
    ...
    checkForm (e) {
      ...
      if (this.name && this.email && this.password && this.repeatPassword) {
        ...
        this.checkIsUserSignedUpStatus(this.isFormSuccess);
        this.checkIsUserLoggedInStatus(this.isFormSuccess);
      }
      if (!this.name) {
